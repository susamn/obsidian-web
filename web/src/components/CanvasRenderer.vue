<template>
  <div class="canvas-viewer w-full h-full overflow-hidden relative bg-[#111]">
    ...
    <div class="controls">
      ...
      <div class="flex items-center justify-center w-12 text-sm text-gray-300">{{ Math.round(scale * 100) }}%</div>
      <button @click="zoomIn" title="Zoom In"><i class="fas fa-plus"></i></button>
      <button @click="resetView" title="Reset View"><i class="fas fa-compress-arrows-alt"></i></button>
    </div>
      ...
      >
        ...
        <div v-if="node.type === 'file'" class="w-full h-full flex flex-col">
          ...
          <div class="file-content bg-[#202020] flex-1">
            <div v-if="isImage(node.file)" class="w-full h-full flex items-center justify-center overflow-hidden">
            ...
            <div v-else class="text-center p-4">
              <i class="fas fa-file-code text-4xl mb-2 text-gray-600"></i>
              <div class="text-xs text-gray-500">{{ getFileName(node.file) }}</div>
            </div>
          </div>
        </div>
        ...
        <div v-if="node.type === 'link'" class="w-full h-full flex flex-col">
          <div class="file-header bg-red-900/20 border-red-900/30">
            ...
            <a :href="node.url" target="_blank" class="hover:underline text-blue-400 truncate">{{ node.url }}</a>
          </div>
...
const parseCanvasData = () => {
  try {
    ...
    if (!trimmedContent || (!trimmedContent.startsWith('{') && !trimmedContent.startsWith('['))) {
      console.warn('[CanvasRenderer] Content does not appear to be JSON:', trimmedContent.substring(0, 100));
      nodes.value = [];
      edges.value = [];
      return;
    }
    ...
    console.log('[CanvasRenderer] Successfully parsed canvas data:', nodes.value.length, 'nodes,', edges.value.length, 'edges');
  } catch (error) {
...
const getColor = (color) => {
  const map = {
    '1': '#ff5555', '2': '#ffb86c', '3': '#f1fa8c',
    '4': '#50fa7b', '5': '#8be9fd', '6': '#bd93f9'
  };
...
/* Obsidian Colors */
.color-1 { --node-color: #ff5555; } /* Red */
.color-2 { --node-color: #ffb86c; } /* Orange */
.color-3 { --node-color: #f1fa8c; } /* Yellow */
.color-4 { --node-color: #50fa7b; } /* Green */
.color-5 { --node-color: #8be9fd; } /* Cyan */
.color-6 { --node-color: #bd93f9; } /* Purple */
